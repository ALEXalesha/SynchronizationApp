...
//             удалена. Отдаём наверх тем же списком, что и закрытые папки;
...
//
// trashFn(absPath, weight) — удаление. Корзины в приложении больше нет: главный
//   процесс передаёт сюда обычное прямое удаление (почему — см. main.js).
//   weight — сколько файлов покрывает вызов: служебная папка уезжает одним
//   действием на всё содержимое. Отчёт этим весом сейчас не пользуется, но он
//   часть договора: вызывающий обязан иметь возможность узнать цену вызова,
//   а закон о весе в invariants.test.js держит обещание честным.
...
async function applyPlan(srcRoot, dstRoot, plan, trashFn, onProgress = () => {}, opts = {}) {
  ...
  // а в конце работы служебная папка стирается целиком — вместе с ними,
  ...
  const doOverwrite = async (entry) => {
    ...
    try {
      if (await copyFile(srcRoot, dstRoot, entry.path, ensureOnce)) {
        ...
        // отложенный оригинал: без этого он был бы стёрт вместе с мусором,
  ...
  // возвращает его на место, а стирается он только в самом конце.
  const doConflict = async (rel) => {
    ...
    // на пять файлов отчитывалась как один удалённый. Цифру эту сейчас никто
    // не показывает, но вес — часть договора applyPlan, и врать он не должен.
  ...
  // Дошли до конца — только теперь оригиналы стираются. Одним действием на всю
  // папку: это на порядок быстрее, чем по файлу. Вернуть их после этого нельзя
  // ниоткуда, поэтому шаг и стоит последним, когда всё остальное уже сделано.
  let trashed = 0;
  ...
  if (parkedCount > 0) {
    if (orphans.length === 0) {
      try {
        // Вес вызова передаём явно: одним действием уезжает всё содержимое папки,
        // и без него вызывающий не отличил бы один удалённый файл от всего запуска.
