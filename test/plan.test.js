...
test('исключения не платят произведением на файлы индекса', async () => {
  // Соседние проверки «лежит ли путь внутри названного узла» давно считают
  // подъёмом по пути, а эта осталась перебором всего списка исключений на каждый
  // путь индекса. Число исключений — это число снятых галочек в дереве, и оно
  // растёт вместе с деревом: сотня снятых веток на двести тысяч файлов даёт
  // произведение, которого не видно нигде, кроме секундомера. Платит за него
  // только тот, у кого включён подсчёт размеров: живой скан до исключённой ветки
  // просто не доходит, а индекс сравнивает полные пути.
  const files = new Map();
  const dirs = new Set();
  for (let i = 0; i < 200000; i += 1) {
    const d = `ветка/под${i % 2000}`;
    dirs.add(d);
    files.set(`${d}/файл${i}.txt`, { size: 1, mtimeMs: 0 });
  }
  const excludes = Array.from({ length: 800 }, (_, i) => `ветка/под${i}`);

  const started = Date.now();
  const scan = scanFromIndex({ files, dirs, skipped: [] }, 'ветка', excludes);
  const spent = Date.now() - started;
...
test('исключения раскладываются по веткам и не платят произведением', async () => {
  // Раскладку индекса по веткам починили, а живой скан рядом остался просить
  // у main.js список исключений на каждую ветку и на каждую сторону — то есть
  // перебирать его целиком. Оба предела растут из одного места, из отметок
  // в дереве: «Выбрать все» даёт тысячи веток, снятые внутри галочки — тысячи
  // исключений. Замер до правки: 5000 веток на 30 тысяч исключений — 88 секунд,
  // и платит за них как раз тот, у кого подсчёт размеров выключен.
  const excludes = [];
  const folders = [];
  for (let i = 0; i < 5000; i += 1) folders.push(`ветка${i}`);
  for (let i = 0; i < 30000; i += 1) excludes.push(`ветка${i % 5000}/под${i}/файл.txt`);

  // Меряем ровно то, что делает запуск: раскладка один раз, а выборка — на каждую
  // ветку и на каждую из двух сторон. Иначе тест мерил бы не тот предел.
  const started = Date.now();
  const byBranch = groupExcludesByBranch(excludes);
  let отобрано = 0;
  for (const f of folders) {
    отобрано += (byBranch.get(f) || new Set()).size;
    отобрано += (byBranch.get(f) || new Set()).size;
  }
  const spent = Date.now() - started;

  assert.strictEqual(отобрано, 60000, 'каждая ветка получила свои исключения дважды — по разу на сторону');

  assert.strictEqual(byBranch.get('ветка0').size, 6, 'все свои исключения у ветки на месте');
  assert.ok(byBranch.get('ветка0').has('под0/файл.txt'), 'путь относительно ветки');
  assert.strictEqual(
    [...byBranch.get('ветка0')].some((p) => p.startsWith('ветка')),
    false,
    'имя ветки в относительном пути не остаётся'
  );
  assert.ok(spent < 2000, `раскладка исключений заняла ${spent} мс — похоже на перебор списка на ветку`);
});

test('исключение достаётся каждой ветке-предку и находится в любом регистре', () => {
  // Вложенная ветка выбирается вместе с родителем (иначе её не включить),
  // и запрет внутри неё виден обеим: подъём обязан отдать исключение всем
  // предкам, а не только ближайшему. Регистр не участвует по общей причине:
  // имя ветки приходит с той стороны, что попала в список первой.
  const byBranch = groupExcludesByBranch(['док/а/б/нельзя.txt', 'верх.txt']);

  assert.deepStrictEqual([...byBranch.get('док/а/б')], ['нельзя.txt']);
  assert.deepStrictEqual([...byBranch.get('док/а')], ['б/нельзя.txt']);
  assert.deepStrictEqual([...byBranch.get('док')], ['а/б/нельзя.txt']);
  assert.strictEqual(byBranch.get('ДОК'.toLowerCase()), byBranch.get('док'), 'ключ без регистра');
  assert.strictEqual(byBranch.get(''), undefined, 'исключение верхнего уровня ветке не принадлежит');
});

test('ветки не платят произведением на обращения к диску', async () => {
  // План разбирает ветки по одной, и на каждую спрашивал у диска тип самой ветки
  // и всей цепочки её родителей. Оба множителя законные: тысячи веток от «Выбрать
  // все» и общий предок у всех — 'год/квартал/…'. Предок читался заново на каждую
  // ветку, а сами обращения шли строго по очереди. На локальном диске это доли
  // секунды, на сетевой шаре с 3 мс на обращение — около минуты тишины ровно
  // между сканом и появлением предпросмотра.
  //
  // Меряем не секундомером, а двумя счётчиками: сколько обращений всего
  // и сколько их держалось в воздухе разом. Секундомер тут врёт — на Windows
  // задержка таймера крупнее самой задержки, и тест падал бы на своей же
  // заглушке, а не на баге.
  const folders = [];
  for (let i = 0; i < 1500; i += 1) folders.push(`год/квартал${i % 4}/ветка${i}`);

  const настоящий = fsp.stat;
  let вызовов = 0;
  let вВоздухе = 0;
  let разом = 0;
  fsp.stat = () => {
    вызовов += 1;
    вВоздухе += 1;
    if (вВоздухе > разом) разом = вВоздухе;
  ...
  const пусто = async () => ({ files: [], dirs: [], skipped: [] });

  try {
    await buildRunPlan('/и', '/п', folders, [], пусто);

    // 1500 веток + 4 квартала + 'год' на две стороны — 3010 разных узлов.
    assert.ok(вызовов <= 3200, `${вызовов} обращений к диску — похоже на чтение предков заново`);
    // Одна ветка сама по себе даёт шесть обращений разом (два узла и две пары
    // родителей), поэтому порог заведомо выше: он ловит именно пачку.
    assert.ok(разом >= 12, `разом всего ${разом} обращений — похоже на очередь по ветке`);
  } finally {
    fsp.stat = настоящий;
  }
});
