...
// planDirs считает 'a0' и 'A0' одной и той же и ничего не создаёт. Разница
// оставалась навсегда, и файл уходил в перезапись на каждом запуске — вечный
// круг: предпросмотр обещает работу, работа выполняется, следующий предпросмотр
// обещает ровно ту же. По сети это перекачка целых веток на каждом прогоне.
test('разный регистр у папки-предка — это не повод перезаписывать файл', () => {
  const plan = planSync([f('док/a0/n.txt', 10, 1000)], [f('док/A0/n.txt', 10, 1000)]);
  assert.strictEqual(plan.overwrite.length, 0);
  assert.strictEqual(plan.unchanged.length, 1);
  assert.strictEqual(plan.copy.length, 0);
  assert.strictEqual(plan.trash.length, 0);
...
// А вот написание самого файла чинится и обязано чиниться: перезапись начинается
// с переноса оригинала в служебную папку, поэтому на месте остаётся имя источника.
test('разный регистр у имени файла внутри такой папки всё равно перезапись', () => {
  const plan = planSync([f('док/a0/Note.txt', 10, 1000)], [f('док/A0/note.txt', 10, 1000)]);
  assert.strictEqual(plan.overwrite.length, 1);
  assert.strictEqual(plan.overwrite[0].path, 'док/a0/Note.txt');
});
